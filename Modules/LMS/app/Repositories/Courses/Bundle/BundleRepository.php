...
class BundleRepository extends BaseRepository
{
    ...
    public function bundleGetByUser($item = 10, $options = [])
    {
        ...
        $fields = ['organization_id' => authCheck()->id];

        // If the user is an instructor, set the options for instructor ID.
        if (isInstructor()) {
            $fields = ['instructor_id' => authCheck()->id];
        }
    ...
    public function bundleEdit($id, $locale = null)
    {
        ...
        // If the user is an instructor, adjust the options to filter by instructor ID.
        if (isInstructor()) {
            $options = ['instructor_id' => authCheck()->id];
        }
