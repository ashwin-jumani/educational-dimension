...
class UserRepository  extends BaseRepository
{
            ...
            ->withWhereHas(
                ...
                function ($query) {
                    $query->saleCountNumber()->orderBy('sale_count_number', 'DESC')
                        ->with('coursePrice', 'instructors', 'levels');
                }
            )->first();

        $data['best_total_course'] = $user->courses ?? 0;
        $user = User::withWhereHas('courses')->where('id', operator: authCheck()->id)->first();
        $coursesId = $user?->courses?->pluck('id')->toArray() ?? 0;
        $data['total_amount'] = PurchaseDetails::whereId('course_id', $coursesId)->sum('price');
        $data['total_course'] = $user?->courses?->count() ?? 0;

        return $data;
    }

    /**
     * dashboardInfoOrganizationd
     */
    public function dashboardInfoOrganization(): array
    {
        ...
        $data['best_total_course'] = $user->organizationCourses ?? null;
        $user = User::withWhereHas('organizationCourses')->where('id', operator: auth()->user()->id)->first();
        $coursesId = $user?->organizationCourses?->pluck('id')->toArray() ?? null;
        $data['total_amount'] = PurchaseDetails::whereId('course_id', $coursesId)->sum('price');
    ...
    public function enrolledStudents()
    {
        ...
        if (isInstructor()) {
            ...
            $bundles =  $user->instructorBundles;
        }
        if (isOrganization()) {
