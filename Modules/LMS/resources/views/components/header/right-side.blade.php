...
<!-- WISH LIST -->
@if ($wishlist['is_show'] ?? false)
    <a href="{{ $wishlist['url'] ?? '#' }}" aria-label="Wishlist icon"
        class="{{ $wishlist['link_class'] ?? 'relative hidden sm:flex-center text-white px-2 py-3 shrink-0' }}">
        <img data-src="{{ $wishlist['icon_image'] ?? asset('lms/frontend/assets/images/icons/wish-list-white.svg') }}"
            alt="Icon">
        <span class="{{ $wishlist['badge_class'] ?? 'flex-center size-6 rounded-50 bg-secondary text-xs text-white absolute top-0 -right-1 rtl:right-auto rtl:-left-1' }}">{{ total_qty() }}</span>
    </a>
@endif
<!-- CART LIST -->
@if ($cart['is_show'] ?? true)
    <a href="{{ $cart['url'] ?? route('cart.page') }}" aria-label="Cart icon"
        ...
        <span class="{{ $cart['badge_class'] ?? 'flex-center size-6 rounded-50 bg-primary text-xs text-white border-2 border-white absolute top-0 -right-1 rtl:right-auto rtl:-left-1 total-qty' }}">{{ total_qty() }}</span>
    </a>
@endif
<!-- ACTION LINK -->
