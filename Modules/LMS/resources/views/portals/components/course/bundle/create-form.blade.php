...
<form action="{{ isset($bundle) ? route($updateRoute, $bundle->id) : route($storeRoute) }}" class="mb-4 form"
    ...
    @endif
    @if (isInstructor())
        <input type="hidden" name="instructor_id" value="{{ authCheck()->id }}">
    @endif
    @if (isOrganization())
    ...
    <div class="grid grid-cols-12 gap-x-4">
        <div class="col-span-full {{ is_active($translate) !== 'active' ? 'lg:col-span-6' : '' }} card">
            ...
            <div class="mt-7">
                ...
                @if (is_active($translate) !== 'active')
                    <div class="mt-4">
                        <label for="bundle-price" class="form-label"> {{ translate('Price') }}($) <span
                                class="text-danger">
                                *</span> </label>
                        <input type="number" id="bundle-price" placeholder="{{ translate('Bundle Price') }}"
                            name="price" class="form-input" value="{{ isset($bundle) ? $bundle->price : '' }}">
                        <span class="text-danger error-text title_err"></span>
                    </div>
                @endif
